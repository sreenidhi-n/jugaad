...
def check_match(face_1_encoding, face_2_encoding):
...
def save_faces(image, bboxes, image_path):
    ...
    existing_subfolders = subfolders(save_dir)
    ...
    for i, box in enumerate(bboxes):
        if len(bboxes) > 1:
            ...
            face1 = face_recognition.face_encodings(face1)[0]
            rem = face_path
            face = image  # Ensure 'face' is defined for later use
        
        saved = False

        for sub in existing_subfolders:
            try:
                random_image_path = get_random_image(os.path.join(save_dir, os.path.join(sub, "Face")))
                tempimg = face_recognition.load_image_file(random_image_path)
                tempimg = face_recognition.face_encodings(tempimg)[0]
                if check_match(face1, tempimg):
